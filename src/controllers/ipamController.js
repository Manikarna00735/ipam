...
async function createPrefix(req, res, next) {
  try {
    ...
    const values = [req.orgid, req.user ? req.user.user_id : null, new Date(), new Date(), payload.status || null, payload.prefix, payload.vrf || null, payload.tenant || null, payload.site || null, payload.vlan || null, payload.role || null];
...
async function createSubnet(req, res, next) {
  try {
    ...
    const values = [req.orgid, prefixId, payload.subnet, payload.status || null, payload.vrf || null, payload.tenant || null, payload.site || null, payload.vlan || null, payload.role || null, req.user ? req.user.user_id : null, new Date(), new Date()];
    const result = await db.query(sql, values);
    const created = result.rows[0];
...
async function createIp(req, res, next) {
  try {
    ...
    const values = [req.orgid, subnetId, payload.ip, req.user ? req.user.user_id : null, new Date(), new Date()];
    const result = await db.query(sql, values);
    const created = result.rows[0];
  ...
  try {
    ...
    try {
      for (const ipAddr of ips) {
        ...
        const r = await db.query('INSERT INTO ips (org_id, subnet_id, ip, created_by, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *', [req.orgid, subnetId, ipAddr, req.user ? req.user.user_id : null, new Date(), new Date()]);
