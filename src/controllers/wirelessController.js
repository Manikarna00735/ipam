...
async function createWireless(req, res, next) {
  try {
    ...
    const sql = `INSERT INTO wireless (name, slug, description, comments, tagscsv, org_uuid, user_uuid, updatedat) 
...
async function listWireless(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM wireless WHERE org_uuid = $1 ORDER BY name', [req.org_uuid])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function getWireless(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM wireless WHERE org_uuid = $1 and uuid = $2 ORDER BY name', [req.org_uuid, req.params.id])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function updateWireless(req, res, next) {
  try {
    ...
    const sql = `UPDATE wireless SET ${setClauses}, updatedat = current_timestamp, user_uuid = $${values.length+1} WHERE uuid = $${values.length+2} RETURNING *`;
