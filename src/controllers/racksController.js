...
async function listRacks(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM racks WHERE org_uuid = $1 ORDER BY name', [req.org_uuid])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function getRack(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM racks WHERE org_uuid = $1 and uuid = $2 ORDER BY name', [req.org_uuid, req.params.id])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
