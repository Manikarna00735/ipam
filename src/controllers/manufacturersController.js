...
async function createManufacturers(req, res, next) {
  try {
    ...
    const sql = `INSERT INTO manufacturers (name, slug, description, comments, tagscsv, org_uuid, user_uuid, updatedat) 
...
async function listManufacturers(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM manufacturers WHERE org_uuid = $1 ORDER BY name', [req.org_uuid])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function getManufacturer(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM manufacturers WHERE org_uuid = $1 and uuid = $2 ORDER BY name', [req.org_uuid, req.params.id])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function updateManufacturer(req, res, next) {
  try {
    ...
    const sql = `UPDATE manufacturers SET ${setClauses}, updatedat = current_timestamp, user_uuid = $${values.length+1} WHERE uuid = $${values.length+2} RETURNING *`;
