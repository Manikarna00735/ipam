...
async function createLocations(req, res, next) {
  try {
    ...
    const sql = `INSERT INTO locations (name, slug, description, comments, tagscsv, org_uuid, user_uuid, updatedat) 
...
async function listLocations(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM locations WHERE org_uuid = $1 ORDER BY name', [req.org_uuid])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function getLocation(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM locations WHERE org_uuid = $1 and uuid = $2 ORDER BY name', [req.org_uuid, req.params.id])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function updateLocation(req, res, next) {
  try {
    ...
    const sql = `UPDATE locations SET ${setClauses}, updatedat = current_timestamp, user_uuid = $${values.length+1} WHERE uuid = $${values.length+2} RETURNING *`;
