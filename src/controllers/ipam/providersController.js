...
async function listProviders(req, res, next) {
  try {
    ...
    const rows = (await db.query('SELECT * FROM providers WHERE orgid = $1 AND deleted_at IS NULL ORDER BY name', [req.orgid])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function getProvider(req, res, next) {
  try {
    const rows = (await db.query('SELECT * FROM providers WHERE orgid = $1 AND uuid = $2 AND deleted_at IS NULL ORDER BY name', [req.orgid, req.params.id])).rows;
    res.json({ items: rows });
  } catch (err) { next(err); }
}
...
async function updateProvider(req, res, next) {
  try {
    ...
    const payload = req.body || {};
    const getRes = await db.query('SELECT * FROM providers WHERE uuid = $1 AND deleted_at IS NULL', [id]);
...
async function deleteProvider(req, res, next) {
  try {
    ...
    const row = getRes.rows[0]; if (!row) return res.status(404).json({ error: 'not found' });
    if (row.orgid !== req.orgid) return res.status(403).json({ error: 'org mismatch' });
    await db.query('UPDATE providers SET deleted_at = NOW() WHERE uuid = $1', [id]);
